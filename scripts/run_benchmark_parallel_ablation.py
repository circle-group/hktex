...
def benchmark_trainable(config):
    ...
    try:
        ...
        n_kernels = 0
        if hasattr(out["optimisation"].model, "_kernel_locations"):
            n_kernels = out["optimisation"].model._kernel_locations.shape[0]
        elif hasattr(out["optimisation"].model, "N_sources"):
            n_kernels = out["optimisation"].model.N_sources
