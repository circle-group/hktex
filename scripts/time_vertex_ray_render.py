...
def timed_render_result_from_trainer(trainer, rotating_frames: int = 10):
    ...
    # Part 2
    mi_mesh = renderer.mesh_to_mitsuba(trainer._train_mesh, trainer.vertex_colours)
...
if __name__ == "__main__":
    ...
    p.add_argument("--config", type=str, default="configs/multiview_vertex_ray.yaml")
    p.add_argument(
        "--base_timing_csv",
        type=str,
    ...
    p.add_argument("--output_dir", type=str, default="outputs/time_vertex_ray_render")
    ...
    print(
        f"Vertex benchmark rows after filtering with base timings: {len(df)}; "
    ...
    for _, r in df.iterrows():
        fname = r["filename"]
        ckpt, cfg = find_ckpt_for_filename(args.benchmark_out_dir, fname)
        if ckpt is None:
            continue
        items.append({"filename": fname, "ckpt_path": ckpt, "config_path": cfg})
    ...
    tuner = tune.Tuner(
        ...
        run_config=tune.RunConfig(
            storage_path=os.path.abspath(args.output_dir), name="time_vertex_ray_render"
        ),
    )
    ...
    output_csv = os.path.join(args.output_dir, "time_vertex_ray_render_results.csv")
