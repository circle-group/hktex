...
from heatsplats.utils import total_variation_loss, smoothness_loss
...
class UvTexturePCLTrainer(UvTextureTrainer):
    ...
    def _compute_edge_aware_photo_loss(
        ...
        edge_norm = edge_score / (edge_score.mean() + float(self.cfg.edge_aware_photo_eps))
    ...
    def _graph_regulariser_with_pair_weights(
        ...
        per_node = (pair_term * pair_weights).sum(dim=1) / (pair_weights.sum(dim=1) + eps)
    ...
    def compute_loss(
        ...
        per_point_loss = self.loss_func(colours, gt_colours, reduction="none").sum(dim=1)
