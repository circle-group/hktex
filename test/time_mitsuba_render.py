...
def timed_render_result_from_trainer(trainer, rotating_frames: int = 10):
    ...
    # Part 1: follow mitsuba_ray_trainer.render_result()
    ...
    prev_mode = None
    if getattr(trainer.cfg, "use_knn_implementation", False):
        prev_mode = trainer.model.inference_mode(True)
        trainer.prepare_knn()
    ...
    if getattr(trainer.cfg, "use_knn_implementation", False):
        trainer.reset_knn()
        trainer.model.inference_mode(prev_mode)
...
def infer_trainable(config):
    ...
    extras = [
        ...
        # Mitsuba benchmark variant wires eigen_albo under trainer.network
        f"trainer.network.eigen_albo.mesh_path={mesh_path}",
        f"trainer.network.eigen_albo.error_if_not_precomputed=True",
    ...
    # Mitsuba-ray trainer wraps HeatKernelTexture under trainer.model.model
    if hasattr(trainer.model, "model") and hasattr(trainer.model.model, "load_torch"):
        trainer.model.model.load_torch(ckpt)
    else:
...
if __name__ == "__main__":
    ...
    tuner = tune.Tuner(
        ...
        run_config=tune.RunConfig(
            storage_path=os.path.abspath(args.output_dir), name="time_mitsuba_render"
        ),
    )
    ...
    output_csv = os.path.join(args.output_dir, "time_mitsuba_render_results.csv")
