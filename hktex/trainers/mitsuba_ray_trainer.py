...
from hktex.modules.heat_kernel_model_knn import HeatKernelModelKNN
from hktex.data import MeshSamplerDataModule
import hktex.rendering as rendering
from hktex.rendering.torch_texture_renderer import TorchTextureRenderer
...
from hktex.utils import ObjectWithCallbacks, load_mesh
...
class MitsubaRayTrainer(ObjectWithCallbacks):
    ...
    def configure(
        ...
        **kwargs,
        ...
        self.model: TextureModel = hktex.find(self.cfg.network_type)(
    ...
    def _get_renderer(self):
        renderer: TorchTextureRenderer = hktex.find(self.cfg.renderer_type)(
            self.cfg.renderer
        )
    ...
    def optimise(self, n_iter=100, debug_log_dir=None):
        ...
        hktex.debug(f"INITIAL -> {self._debug_opt_params_string()}")
        ...
        for epoch in (pbar := tqdm(range(n_iter), disable=True)):
            ...
            if hktex.is_debug():  # and (epoch == 0 or (epoch + 1) % 10 == 0):
        ...
        hktex.debug(f"FINAL -> {self._debug_opt_params_string()}")
...
@hktex.register("trainers.uv-texture-mitsuba-ray")
